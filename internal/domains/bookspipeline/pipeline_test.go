...
func TestRunPipeline_OrderAndAggregation(t *testing.T) {
	...
	// Steps MUST run in dependency order: ingests, then match, then pull.
	...
	if sum.AudibleSynced != 3 || sum.KindleSynced != 5 || sum.Matched != 7 || sum.Pulled != 9 {
		t.Fatalf("summary counts = %+v, want {3,5,7,9}", sum)
	}
	if len(sum.Errors) != 0 {
