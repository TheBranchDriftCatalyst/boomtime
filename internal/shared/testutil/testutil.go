...
//
// boom-zp2s: the boomtime DATA-domain handler bags moved off *handler.Handler onto
...
func (hz *Harness) Cleanup(sender string) {
	...
	hz.T.Cleanup(func() {
		for _, q := range []string{
			// boom-d6x.handler: health_samples + health_rollup_daily are
