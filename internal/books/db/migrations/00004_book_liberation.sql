...
-- 00004_book_liberation.sql — the standalone catalyst-books copy of the host's
-- liberation state (host migration 00082). Byte-identical EXCEPT that
-- book_liberation_attempts.owner is a PLAIN column rather than a FK to
-- public.users: the standalone runs single-owner against its own books-only
-- database and has no full users table (see internal/books/db/migrations.go).
--
