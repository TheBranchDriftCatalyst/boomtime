...
export function makeBooksExplorerConfig(
  ...
  return {
    ...
    // The Hardcover status badge (in the Hardcover column) is itself the per-row
    // sync button — click it to push effective→Hardcover inline and clear the
    // divergence in place. The trailing action is just the open-on-Hardcover link.
    rowActions: (r) => (
      <button
        type="button"
        onClick={(e) => {
