...
var _ = Describe("RegenerateAvatar happy path (gaka-d6x.handler)", func() {
	It("dispatches the async goroutine (202 immediate) and eventually saves the bytes", func() {
		// Mock comfyui shim so the goroutine calls it and succeeds.
		...
		e := hz.Router()
		_, token := hz.MintUser("avregen_ok")
...
var _ = Describe("SynthesizePrompt success path w/ mock LLM (gaka-d6x.handler)", func() {
	...
	It("returns 502 when the LLM upstream responds non-200", func() {
