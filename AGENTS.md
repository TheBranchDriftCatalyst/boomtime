...
encrypted Wakatime-key feature (boom-6jm.2). Generate with
`openssl rand -base64 32` and set it in `.env`.

- `BOOM_ENV=dev`/`test`: missing key WARNs at boot; feature stays inert.
- `BOOM_ENV=prod`/`production`: missing key EXITS at boot (boom-6jm.9).

Rotate the key with `boomtime rotate-encryption-key --old <b64> --new <b64>`
(boom-6jm.7) BEFORE flipping the env value; a hot swap strands existing
