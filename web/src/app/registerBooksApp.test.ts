...
// @/lib/standalone folds into a module-level const at load time. So each test
...
import type { RouteDef } from "@/shared/routing/types";
...
async function composeStandalone() {
  const nav = await import("@/shared/nav/registry");
  const routing = await import("@/shared/routing/registry");
  nav.__resetNavRegistry();
  routing.__resetRouteRegistry();
  const { registerBooksAppDomains } = await import("@/app/registerBooksApp");
