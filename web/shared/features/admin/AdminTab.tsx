...
import { useLabelsCatalog } from "@shared/features/publicprofile/labels/useLabelsCatalog";
import type { LabelCatalogRow } from "@shared/features/publicprofile/labels/types";
import { LabelImage } from "@shared/features/publicprofile/labels/LabelImage";
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-md border border-border bg-card p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Which queue runs regens. There used to be two — the server's
              in-process pool, or a separate boomtime-worker pod pulling off
              RabbitMQ, with a queue-depth readout and a link to the broker's
              mgmt UI. Both are gone (boom-piig phases 2-3): regen is an ordinary
              catalyst-go-jobs kind now. Queue depth is a first-class metric
              (jobs_queue_depth) rather than an ad-hoc field on this endpoint,
              so this is a plain label. */}
          <span
            ...
            title="Regens run on the shared job queue (catalyst-go-jobs) — see Admin › Jobs"
          >
            <Network size={10} />
...
function StreakBackfillSection() {
  ...
  async function runBackfill() {
    ...
    setError(null);
    try {
      const res = await api.awardsBackfill(days);
      setResult(res);
    } catch (e) {
    ...
    <section className="rounded-md border border-border bg-card p-4">
      ...
      <div className="mt-4 flex items-end gap-3">
        ...
        <Button
          ...
          disabled={running}
        >
          {running ? "▸ RUNNING…" : `▸ BACKFILL ${days} DAYS`}
        </Button>
        {result && (
        ...
        {error && (
          <div className="flex-1 text-[11px] font-mono text-destructive">
            {error}
          </div>
        )}
      </div>
